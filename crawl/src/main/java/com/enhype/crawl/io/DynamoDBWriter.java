...
public class DynamoDBWriter {
	...
	private Logger logger = Logger.getLogger(DynamoDBWriter.class.getName());
	...
	public void writeSite(String entry, long startPageSeq, long endPageSeq, long sentSeq) {
		...
		siteAttrs.put("sent_num", Long.toString(sentSeq));
	...
	public void writeEntities(WebPage page) {
		...
		for (String entityURI : page.getEntityMentionMap().keySet()) {
			...
			entityUpdates.put("surfaces", new ArrayList(
					Crawler.getEntitiesSurfaceList().get(entityURI)));

			if (!Crawler.getKnownEntities().contains(entityURI)) {
				...
				if (!exist) {
					...
					if (Crawler.getEntitiesList().get(entityURI).length() > 0)
						entityUpdates.put("types",
								Arrays.asList(StringUtils.split(
										Crawler.getEntitiesList().get(entityURI),
										",")));

					dynamo.addItem(entityTableName, entityAttrs, entityUpdates);
					Crawler.getKnownEntities().add(entityURI);
